...
class StateManager:
    ...
    def return_init(self):
        ...
        self.table_setup_style = dash.no_update
        

    def exploratory_selection(self):

        self.start_page_style = {"display":"none"} # Hide the start_page as we move to next page
        self.fig = GPlots.plot_global_country(background_color = APP_BACKGROUND_COLOR).update_layout(clickmode='event+select')
        self.fig_style = {**Setup.geofig_styling, "backgroundColor":APP_BACKGROUND_COLOR}
        self.step = "maps1"
    ...
    def targeted_selection(self):
        self.start_page_style = {"display":"none"} # Hide the start_page as we move to next page
        self.step = "tables"
        self.table_setup_style = {'display': 'flex',"alignItems": "center",
                                  "justifyContent": "center","flexDirection": "column"}
        self.full_rows = df_table.head(1000).to_dict("records")

    def portfolio_page(self, initial = True):

        if initial == True:
            # Convert selected rows to df names for filtering
            self.selected_companies = [i["Names"] for i in self.selected_rows]
            # Empty the selected_rows to reduce memory usage
            self.selected_rows = []
            self.df_names = [] # Empty it because it is no longer used by the app

        self.grid_output = AppSetup.portfolio_page_generator(df_global[df_global["Names"].isin(self.selected_companies)].copy())
    ...
    def return_list(self):
        ...
        return [
            ...
            self.table_setup_style, self.full_rows,
            self.selected_rows
            ]
...
## Full dataframe for table displays
df_table = df_global[["Names", "Country", "Sector", "Industry", "Size", "SalesP", "AssetP", "BorrowP", "ShrinkP"]].copy()
df_table = AppSetup.table_prep(df_table)
...
app.layout = html.Div(
    ...
    # Data table that can be trigerred via targeted search or table mode in exploratory search.
    html.Div(
        id = "table-setup",
        style = {"display":"none"},
        children = [
            html.H1("", id = "main-table-header", style = {"text-align":"center"}),
            dag.AgGrid(
                id = "main-table",
                columnDefs = AppSetup.table_column_definitions(),
                rowData = [],
                dashGridOptions = {
                    "pagination": True,
            ...
            html.Div([
                html.H2("Selected Portfolio", id = "selected-table-header",
                        style = {'text-align': 'center', 'display': 'inline-block'}),
                html.Button("Done", id = "portfolio-selection-button", n_clicks = 0,
                           style = {
                               "margin-left": "20px",
                                "padding": "8px 16px",
                                "font-size": "1em",
    ...
    html.Button('Okay', id = 'okay-button', 
                n_clicks = 0, style = {"display":"none"}), # This block is initially hidden
...
@app.callback(
    [
        ...
        Output('table-setup', 'style'),
        Output('main-table', 'rowData'),
        Output('selected-table', 'rowData'),
    ...
    [
        ...
        Input("portfolio-selection-button", "n_clicks"),
        Input("main-table", "selectedRows"),
        Input("selected-table", "selectedRows"),
        Input("main-table", "filterModel"),
    ]
)
def update_app(stored_data,
    ...
    if exploratory_clicks and SM.curr_step == "startPage":
        ...
        SM.targeted_selection()

    elif SM.curr_step == "tables" and ctx.triggered[0]['prop_id'] == "main-table.filterModel":

        df_temp = df_table.copy()

        for column, filter_info in main_table_filter.items():

            if "filter" in filter_info:
                filter_value = filter_info["filter"]
                # Apply filter to each column as case-insensitive substring search
                df_temp =  df_temp[df_temp[column].str.contains(filter_value, case=False, na=False)]
                
        
        SM.full_rows = df_temp.head(1000).to_dict("records")
        del df_temp


    # Why is there no filterModel for selected-table?
    # Because we always display all the rows for it so dont need to update anything on our end
        

    elif SM.curr_step == "tables" and triggered_id == "main-table" and main_table_selection:

        if main_table_selection[0] not in SM.selected_rows:
            SM.selected_rows.append(main_table_selection[0])

    elif SM.curr_step == "tables" and triggered_id == "selected-table" and selected_table_selection:
        # Remove selected rows
        SM.selected_rows = [row for row in SM.selected_rows if row != selected_table_selection[0]]
        
    elif SM.curr_step == "tables" and portfolio_confirm_clicks:
        SM.full_rows = [] # Empty full table rows
        # Update df_names using selected_rows
        # Hide tables page
        # Update step to portfolio_page
        SM.table_setup_style = {"display":"none"}
        SM.step = "portfolio_page"
        SM.portfolio_page(initial = True)
