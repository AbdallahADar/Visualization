...
#     return html.Div([
#         html.Button('Sales', id='button-sales', n_clicks=0, style = model_mode_button_style(model_mode == "Sales")),
#         html.Button('Asset', id='button-asset', n_clicks=0, style = model_mode_button_style(model_mode == "Asset")),
#         html.Button('Borrow', id='button-borrow', n_clicks=0, style = model_mode_button_style(model_mode == "Borrow")),
#         html.Button('Shrink', id='button-shrink', n_clicks=0, style = model_mode_button_style(model_mode == "Shrink"))
...
## Initial Category button set up
...
def initial_bubble_elements(df):
...
def bucket_dist_cards(company_data, page):

    return html.Div([
        html.Div([
            dcc.Graph(figure = DMPlots.bucket_distribution(company_data,
...
def risk_segmentation_cards(company_data, color_mapping, position_mapping, order_scenario, label_scenario, segment_names, background_color):
    ...
    #         html.Div(
    #             children = label, 
    #             style = {**SEGMENT_LABEL, 'height': '30%', 'display': 'flex', 'alignItems': 'center', 'justifyContent': 'center'}),
    #     ], style = {
    #     'flex-basis': '35%',  # Takes 35% of the width
    ...
    #     # Plot
    #     html.Div([
    #     dcc.Graph(figure = CGPlots.classification_grid_plot(out, color_mapping, position_mapping, background_color),
    #              config={'responsive': True})
    #         ], style = {'flex-basis': '65%',
    #                     # 'height': '70%',  # Set height to 500px (square dimension)
    #                     # 'width': '350px',  # Set width to match height (square dimension)
    #                     'margin': '0px',  # Remove margins from all sides
    #                     'flex-shrink': '1',
    #                     'max-height': '90%',
    #                     # 'max-width': '100%',
    #                     'alignItems': 'center',  # Center align the column content
...
def venn_narrative_cards(company_data, narrative, background_color, order_scenario, color_mapping, segment_names):
    ...
    #              style={'height': '450px', 'width': '400px', 'padding': '10px'}),
    ...
    #         'width': '450px',  # Same width as the image for balance
...
def company_page_generator(company_data, df, page, model_type_list):
    ...
    row6 = html.Div([
        ...
        bucket_dist_cards(company_data, page),
    ], style={'display': 'flex','width': '100%'})
