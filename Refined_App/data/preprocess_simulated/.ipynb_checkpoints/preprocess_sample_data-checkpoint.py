...
for country in non_granular_countries:
    for size in sizes:
        for sector in SECTORS:
            ...
            for n in range(nobs):
                data.append([country, size, sector, country])


df_non_granular = pd.DataFrame(data, columns = ["Country", "Size", "Sector", "Location"])
...
for geo in nuts["NUTS3_ID"].unique():
    for size in sizes:
        for sector in SECTORS:
            nobs = np.random.randint(5,50)
            for n in range(nobs):
                data2.append([geo, size, sector])

df_nuts = (
    pd.DataFrame(data2, columns = ["NUTS3_ID", "Size", "Sector"])
...
for geo in df_counties["COUNTY_ID"].unique():
    for size in sizes:
        for sector in SECTORS:
            nobs = np.random.randint(5,50)
            for n in range(nobs):
                data3.append([geo, size, sector])

df_us = (
    pd.DataFrame(data3, columns = ["COUNTY_ID", "Size", "Sector"])
