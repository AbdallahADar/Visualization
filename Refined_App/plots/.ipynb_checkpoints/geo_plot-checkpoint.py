...
def geo_plot(sector, state, bg_color):
    ...
    if state.step == "countries":
        fig = COUNTRY_CHOROPLETH(f"{PATH}data/geo_rates/{sector}/{file_name}", bg_color)
    
    elif state.step == "us_state":
        fig = US_STATE_CHOROPLETH(f"{PATH}data/geo_rates/{sector}/{file_name}", bg_color)

    elif state.step == "us_county":
        fig = US_COUNTY_CHOROPLETH(f"{PATH}data/geo_rates/{sector}/{file_name}", state.us_state, bg_color)
...
def geo_plot_risk(risk_type, bg_color):
    ...
    fig = COUNTRY_CHOROPLETH_RISK(f"{PATH}data/risk_scores/{risk_type}/{file_name}", risk_type, bg_color)
